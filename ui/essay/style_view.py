...
def render_style_view(repo: EssayRepository):
    st.markdown("### 문체 편집")
    st.caption("내 초안을 원하는 문체로 다듬고, 사실과 수치가 그대로인지 확인한 뒤 새 버전으로 저장합니다.")
    ...
    if "latest_rewrite_proposal" in st.session_state:
        ...
        with col_out1:
            ...
            st.info(in_rev.body_text if in_rev else "(원본 조회 불가)")

        with col_out2:
            st.markdown("##### 수정안")
            st.success(proposal.output_text)
