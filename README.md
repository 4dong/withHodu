...
호두 캐릭터 제작 자료: [도트 설계 노트](docs/design/hodu-pixel-design-notes.md) · [도트 레퍼런스 이미지](assets/hodu/hodu-pixel-reference-v1.png). 실제 사진을 바탕으로 외형과 작업별 동작을 정리한 프레임 애니메이션 제작 기준입니다.

화면 제작 자료: [전체 화면·로딩·전환 설계](docs/design/withhodu-screen-storyboard.md) · [시작 화면 검증 기록](docs/design/withhodu-home-verification.md). 시작 화면은 구현했으며, 나머지 작업 화면의 호두 테마와 프레임 애니메이션은 후속 적용 예정입니다.
...
- **대역 리더:** 왼쪽에는 PDF 원문, 오른쪽에는 문단별 한국어 번역을 보여 줍니다. 원문 문단과 번역 문단이 서로 하이라이트로 연결되고, 수식은 KaTeX로 표시합니다.
- **번역 모델:** 키 없이 쓰는 Google 번역, 또는 Gemini(API 키 필요) 중에서 고릅니다. 페이지 단위로 번역하고, 필요하면 현재 페이지만 다시 번역합니다.
- **읽기 편의:** 대역·원문·번역 보기 전환, 글자 크기와 줄 간격 조절, 화면 양 끝에 마우스를 올려 쪽 넘기기, 현재 페이지 내용을 바탕으로 논문에 질문하기.
